...
app.get("/project/:company_id", async (req, res) => {
  ...
  const getallproject = "SELECT * FROM project WHERE company_id_project = ?";

  try {
    const [results] = await (
      await connection
    ).execute(getallproject, [company_id]);
    res.send(results);
  } catch (error) {
...
app.post("/AddEmpProj/:proj_id", async (req, res) => {
  ...
  try {
    ...
    if (error.code === "ER_DUP_ENTRY") {
      res.status(409).json({ error: "Duplicate entry: Employee ID already exists" });
    } else {
      res.status(500).json({ error: "Failed to insert" });
    }
    console.log(error);
...
app.delete("/deleteEmp/:employee_id", async (req, res) => {
  ...
  try {
    const [result] = await (
      await connection
    ).execute(deleteQuery, [employee_id]);
    res.json({ message: "Leave deleted successfully" });
  } catch (error) {
    res.status(500).json({ error: "Failed to delete leave" });
    console.log(error);
